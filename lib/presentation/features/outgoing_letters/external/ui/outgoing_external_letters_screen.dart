...
import 'package:foe_archiving/presentation/features/income_letters/external/ui/external_letters_list.dart';
import 'package:foe_archiving/presentation/features/outgoing_letters/external/ui/otugonig_external_letters_list.dart';
...
import '../bloc/outgoing_external_letters_cubit.dart';
import '../bloc/outgoing_external_letters_states.dart';

class OutgoingExternalLettersScreen extends StatelessWidget {
  const OutgoingExternalLettersScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<OutgoingExternalLettersCubit>()..setupScrollController(context)..loadDefaultLetters(),
      child: BlocConsumer<OutgoingExternalLettersCubit,OutgoingExternalLettersStates>(
        listener: (context, state){},
        builder: (context, state){
          var cubit = OutgoingExternalLettersCubit.get(context);
          var commonDataCubit = sl<CommonDataCubit>();

          return Scaffold(
            appBar: AppBar(
              centerTitle: true,
              title: Text(AppStrings.incomeExternalLetters.tr(),
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height:  AppSize.s16,),
                   Expanded(
                    child: OutgoingExternalLettersListView(letterCubit: cubit,),
                  ),
                ],
              )
            ),
...
Widget searchAndFilterWidget(BuildContext context, OutgoingExternalLettersCubit cubit){
  return Container(
...
Widget headerItem(BuildContext context, OutgoingExternalLettersCubit cubit){
  return Padding(
