...
/// {@endtemplate}
class MPdfThumbnail extends StatefulWidget {
  //@TODO
  // factory PdfThumbnail.fromAsset(String asset) {
  //   return PdfThumbnail._();
  // }

  /// Creates a [MPdfThumbnail] from a file.
  factory MPdfThumbnail.fromFile(
      String path, {
        Key? key,
  ...
  @override
  State<MPdfThumbnail> createState() => _MPdfThumbnailState();
}

class _MPdfThumbnailState extends State<MPdfThumbnail> {
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                return GestureDetector(
                  onTap: () => widget.onPageClicked?.call(0),
                  child: Padding(
                    padding: const EdgeInsets.all(0),
                    child: Stack(
                      children: [
                        DecoratedBox(
                          key: Key('thumbnail_${1}'),
                          decoration:  widget.currentPageDecoration!,
                          child: Image.memory(images[1]!),
                        ),
                        widget.currentPageWidget!(1, true),
                      ],
                    ),
                  ),
                );
